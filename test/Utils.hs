module Utils
  ( allImportDeclarations, -- added by thma
    ModuleImportDeclarations, -- added by thma
    parseFile,
    parseString,
    Qualifier,
    qualifierNodes,
...
import Language.Haskell.Lexer (PosToken, Token (..), lexerPass0, line)
...
parseFile f =
  ...
  where
    get_text txt = if ext == ".lhs" then delit txt else txt
    ext = takeExtension f
...
splitQualifier cs = case break ('.' ==) cs of
  (xs, _ : ys) ->
    let Hierarchy qs = splitQualifier ys
     in Hierarchy (xs : qs)
  _ -> Hierarchy [cs]
...
splitModName cs = case break ('.' ==) cs of
  (xs, _ : ys) ->
    let (Hierarchy as, bs) = splitModName ys
...
relPaths (q, y) = [prefix ++ suffix | suffix <- suffixes]
  where
    prefix = foldr (</>) y (qualifierNodes q)
